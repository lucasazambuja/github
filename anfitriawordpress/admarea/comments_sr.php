...
if (((isset($_GET['delete'])) && ($_GET['delete'] != "")) || ((isset($_GET['deleteparent'])) && ($_GET['deleteparent'] != ""))) {

  $deleteSQL = sprintf("DELETE FROM wp_comments WHERE comment_ID=%s",

                       GetSQLValueString($_GET['delete'], "int"));

 $deleteSQL1 = sprintf("DELETE FROM wp_comments WHERE comment_ID=%s",

                       GetSQLValueString($_GET['deleteparent'], "int"));



  mysql_select_db($database_BLOG_MATRIZ, $BLOG_MATRIZ);

  $Result = mysql_query($deleteSQL, $BLOG_MATRIZ) or die(mysql_error());
  $Result1 = mysql_query($deleteSQL1, $BLOG_MATRIZ) or die(mysql_error());
...
$query_comments = "SELECT * FROM wp_comments WHERE comment_parent != 0 ORDER BY comment_ID DESC";
...
<?php do { 
    ...
    global $wpdb;
    $comment = $wpdb->get_row('SELECT * FROM wp_comments where comment_ID = '.$row_comments['comment_parent'].' LIMIT 0,1');
    ?>
...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="1">
    ...
    <tr>
        <td width="37%" align="center" bgcolor="#F4F4F4" class="Maiusc"><strong><?php echo $comment->comment_author; ?></strong></td>
      <td width="55%" align="center" bgcolor="#F4F4F4" class="style5"><?php echo $comment->comment_author_email; ?></td>
      <td bgcolor="#F4F4F4" class="style5">
      
      <?php
if ($comment->comment_approved == 0) {;?>
      <a href="ativa.php?post_id=<?php echo $comment->comment_ID; ?>&voltar=http://www.anfitria.com.br<?php printf("%s?pageNum_comments=%d%s", $currentPage, min($totalPages_comments, $pageNum_comments), $queryString_comments); ?>"><img src="ativa.gif" width="109" height="18" border="0" /></a>

<?php } else {
    echo "Já Ativo";
	echo "<br>";
	echo "<a href=\"encaminhar.php?post_id=".$comment->comment_ID."&unica=".$comment->comment_post_ID."\" target=\"_blank\" ><img src=\"encaminhar.gif\" width=\"109\" height=\"18\" border=\"0\" /></a>";
}
?>
    ...
    <tr>
      ...
      <td colspan="2" align="left" bgcolor="#F4F4F4" class="Maiusc"><span class="style5" onclick="MM_openBrWindow('<?php echo get_permalink($row_comments['comment_post_ID']); ?> ','pop','status=yes,menubar=yes,scrollbars=yes,resizable=yes,width=600,height=400')" style="cursor:pointer;"><?php echo iconv('utf8','latin1',$comment->comment_content); ?></span></td>

      <td width="8%" bgcolor="#F4F4F4" class="style5"><a href="?delete=<?php echo $comment->comment_ID; ?>&deleteparent=<?php echo $row_comments['comment_ID']; ?>" onClick="return (confirm('Voc&ecirc; realmente quer deletar?'))"><img src="del.gif" width="109" height="18" border="0" /></a></td>

    </tr>
    <tr>
      <td colspan="3" align="center" bgcolor="#F4F4F4" class="Maiusc" ><form id="form1" name="form1" method="POST" action="responde.php">
        <input name="email" type="hidden" id="email" value="<?php echo $comment->comment_author_email; ?>" />
        <input name="post_id" type="hidden" id="post_id" value="<?php echo $comment->comment_post_ID; ?>" />
        <input name="post_parent" type="hidden" id="post_parent" value="<?php echo $comment->comment_ID; ?>" />
        <textarea name="resposta" id="resposta" cols="50" rows="4" style="width:80%;"><?php
if ($row_comments['comment_content'] == '') {; ?><?php echo ucwords($comment->comment_author); ?>
,   beijos<?php } else {
 echo $row_comments['comment_content'];
}
?></textarea>
